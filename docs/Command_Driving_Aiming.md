...
You can look at these templates, or simply fork your copy of them, right on GitHub (whey are public, with WPI license).
...
class FollowObject(commands2.Command):
    ...
    def initialize(self):
        self.finished = False
        self.drivingAllowed = (self.stepSeconds > 0)  # driving is allowed if we are allowed to make steps forward
        self.minDetectionIndex = None
    ...
    def execute(self):
        ...
        if self.subcommand is None or Timer.getFPGATimestamp() > self.halfStepTime:
            ...
            if directionInfo is not None:
                subcommand = self.makeSubcommand(directionInfo)
    ...
    def makeSubcommand(self, directionInfo):
        ...
        if abs(degreesFromTarget) < FollowObject.ANGLE_TOLERANCE and self.drivingAllowed:
            ...
            speed = self.speed
            if self.stopWhen is not None:  # if pretty close to the object, drive slower
                speed = speed * self.stopWhen.reduceSpeedIfClose(x, y, max(size), smoothness=self.smoothness)
            drive = AimToDirection(direction.degrees(), self.drivetrain, fwd_speed=max([speed, FollowObject.MIN_SPEED]))

            newSubcommand = drive.withTimeout(self.stepSeconds)  # correct timeout for the step
        else:
    ...
    def setSubcommand(self, subcommand):
        ...
        self.halfStepTime = Timer.getFPGATimestamp() + self.stepSeconds / 2  # half a step later look again
